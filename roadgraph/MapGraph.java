			...
			if (!visited.contains(curr)) {
				...
				if (curr == endNode) {
 				...
 				for (MapNode neighbor: curr.getNeighbors()) {
 					if (!visited.contains(neighbor)){
 						double distance = curr.getDistanceFromStart() + curr.getEdgeLength(neighbor);
 						if (distance < neighbor.getDistanceFromStart()) {
 							neighbor.setDistanceFromStart(curr.getDistanceFromStart() + curr.getEdgeLength(neighbor));
 							neighbor.setPriorityFunction(neighbor.getDistanceFromStart());
...
//		nodeSearched.accept(next.getLocation());
		...
		HashMap<MapNode, MapNode> parentMap = new HashMap<MapNode, MapNode>();
		boolean found = aStarSearchHelper(startNode, endNode, parentMap, nodeSearched);
	...
	private boolean aStarSearchHelper(MapNode startNode, MapNode endNode, 
			HashMap<MapNode, MapNode> parentMap,
			Consumer<GeographicPoint> nodeSearched) {
		boolean found = false;
		PriorityQueue<MapNode> q = new PriorityQueue<MapNode>();
		...
		while(!q.isEmpty()) {
			...
			if (!visited.contains(curr)) {
				visited.add(curr);
				nodeSearched.accept(curr.getLocation());
//				System.out.println("Adding this node to the visited set: " + curr);
				visitedCount += 1;
				if (curr == endNode) {
					found = true; 
					System.out.println("This is the number of nodes visited"
							+ "by aStar: " + visitedCount);
					System.out.println(visited);
					return found;
				}
				// choose lowest distance neighbor
 				for (MapNode neighbor: curr.getNeighbors()) {
 					if (!visited.contains(neighbor)){
 						double distanceFromStart = curr.getDistanceFromStart() + curr.getEdgeLength(neighbor);
...
// 						System.out.println("This is the getDistance:  " + curr.getDistanceFromStart());
// 						System.out.println("This is the distance to the neighbor:  " + curr.getEdgeLength(neighbor));;
// 						System.out.println("This is the distance from start:  " + distanceFromStart);
// 						System.out.println("This is the neighbor distance from start: " + neighbor.getDistanceFromStart());
// 						System.out.println("This is the distance from goal: " + distanceFromGoal);
 						if (distanceFromStart < neighbor.getDistanceFromStart()){
 							neighbor.setDistanceFromStart(distanceFromStart);
 							neighbor.setPriorityFunction(totalDistance);
 							parentMap.put(neighbor, curr);
// 							System.out.println("Putting neighbor into parentMap: " + neighbor);
// 							System.out.println(parentMap.entrySet());
 							q.add(neighbor);
// 							System.out.println("\nThis is the total distance: " + totalDistance);
...
//		
//		System.out.println(theMap.dijkstra(start, end, null));
//		
//		System.out.println("Here is the aStar search: " );
//		System.out.println(theMap.aStarSearch(start, end, null));
...
//		List<GeographicPoint> route = theMap.dijkstra(start,end);
//		List<GeographicPoint> route2 = theMap.aStarSearch(start,end);
//		System.out.println("This is the first route: \n" + route);
//		System.out.println("This is the second route: \n" + route2);
//		
